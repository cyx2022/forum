...
//        CommonResult<?> result;
        ...
        if (ex instanceof TokenException) {
            respBean = responseStatusExceptionHandler(exchange, (TokenException) ex);
        } else {
            respBean = defaultExceptionHandler(exchange, ex);
        }

        // 返回给前端
        return WebFrameworkUtils.writeJSON(exchange, respBean);
    }
...
//    }
    private RespBean responseStatusExceptionHandler(ServerWebExchange exchange, TokenException ex){
        ServerHttpRequest request = exchange.getRequest();
        return RespBean.error("网关鉴权失败");
    }
...
//    }
    @ExceptionHandler(value = Exception.class)
    public RespBean defaultExceptionHandler(ServerWebExchange exchange,Throwable ex) {
        return RespBean.error("网关鉴权失败");
    }
}
